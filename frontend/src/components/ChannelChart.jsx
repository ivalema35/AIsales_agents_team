  ...
  return (
    <div className="flex flex-col gap-4">
          ...
          {channels.map((c) => {
            ...
            return (
              <div key={c} className="flex items-center gap-2">
                ...
                <span className="w-8 shrink-0 text-right font-mono text-xs font-semibold tabular-nums text-ink-700">{value}</span>
              </div>
            );
          })}
        </div>
      ...
      <div className="mt-1 grid grid-cols-2 gap-3 border-t border-line pt-3">
        {channels.map((c) => (
          <div key={c} className="flex items-center gap-2.5 rounded-md bg-parchment-raised-2 px-3 py-2">
            <span className="h-2 w-2 shrink-0 rounded-full" style={{ backgroundColor: colors[c] }} />
            <div className="flex flex-1 items-center justify-between gap-2">
              <div>
                <p className="text-[11px] text-ink-500">{c === "EMAIL" ? "Email" : "WhatsApp"} seen rate</p>
                <p className="font-mono text-sm font-semibold text-ink-900">
                  {data[c].seen_rate != null ? `${(data[c].seen_rate * 100).toFixed(1)}%` : "—"}
                </p>
              </div>
              <div>
                <p className="text-[11px] text-ink-500">Reply rate</p>
                <p className="font-mono text-sm font-semibold text-ink-900">
