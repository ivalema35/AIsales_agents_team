...
function KanbanColumn({ col }) {
  ...
  return (
    <div className="flex w-80 shrink-0 flex-col rounded-lg bg-parchment-raised ring-1 ring-line">
      <div className="flex items-center justify-between rounded-t-lg border-b border-line bg-parchment-raised px-3 py-2.5">
        ...
        <span className="rounded-full bg-parchment-raised-2 px-2 py-0.5 text-xs font-medium text-ink-500">
          {col.leads.length}
        </span>
      </div>
      ...
      <div className="flex flex-col gap-1.5 overflow-y-auto p-2" style={{ height: `${LIST_HEIGHT_PX}px` }}>
        {col.leads.length === 0 && (
          <p className="px-1 py-4 text-center text-xs text-ink-500/40">No leads</p>
        )}
        {pageLeads.map((lead) => (
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between border-t border-line px-2 py-1.5">
          <button
            ...
            disabled={page <= 1}
            className="rounded px-2 py-1 text-xs font-medium text-ink-500 hover:bg-parchment-raised-2 hover:text-ink-900 disabled:cursor-not-allowed disabled:opacity-30"
          >
            ‹ Prev
          </button>
          <span className="font-mono text-[11px] text-ink-500">Page {page} of {totalPages}</span>
          <button
...
export default function PipelineKanban({ leads }) {
  ...
  return (
    <div className="flex gap-4 overflow-x-auto rounded-xl border border-line bg-parchment p-3">
      {byStatus.map((col) => (
