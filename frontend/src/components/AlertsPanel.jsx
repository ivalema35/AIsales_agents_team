  ...
  return (
    <div className="rounded-xl border border-alert-600/20 bg-alert-100/60 p-4">
      <div className="mb-3 flex items-center gap-2">
        ...
        <h2 className="font-display text-sm font-semibold text-alert-700">Needs your attention</h2>
        <span className="rounded-full bg-alert-100 px-2 py-0.5 text-xs font-semibold text-alert-700">{total}</span>
      </div>

      {error && <p className="mb-2 text-xs text-alert-600">{error}</p>}
      {total === 0 && <p className="text-sm text-alert-700/60">Nothing waiting right now.</p>}

      {needsResponse.length > 0 && (
        <div className={readyToClaim.length > 0 ? "mb-4" : ""}>
          <p className="mb-1.5 text-[11px] font-semibold uppercase tracking-wide text-alert-700">
            Just replied — needs response <span className="text-alert-600/60">({needsResponse.length})</span>
          </p>
      ...
      {readyToClaim.length > 0 && (
        <div>
          <p className="mb-1.5 text-[11px] font-semibold uppercase tracking-wide text-warm-700">
            Ready to claim <span className="text-warm-600">({readyToClaim.length})</span>
          </p>
          <div className="flex flex-col gap-2">
            {readyToClaim.slice(0, CLAIM_PREVIEW_CAP).map((a) => (
              <div
                ...
                onClick={() => openLead(a.lead_id)}
                onKeyDown={(e) => { if (e.key === "Enter") openLead(a.lead_id); }}
                className="flex cursor-pointer items-center justify-between gap-3 rounded-lg border-l-[3px] border-warm-600 bg-parchment-raised p-3 shadow-sm ring-1 ring-warm-600/20 transition-shadow hover:shadow focus:outline-none focus:ring-2 focus:ring-warm-600/30"
              >
                <div className="min-w-0">
                  <p className="truncate text-sm font-medium text-ink-900">{a.company_name}</p>
                  <p className="mt-0.5 truncate text-xs text-ink-500">{a.justification}</p>
                  <div className="mt-1.5 flex items-center gap-2 text-[11px] text-ink-500">
                    <span className="rounded bg-warm-100 px-1.5 py-0.5 font-semibold text-warm-700">Score {a.score}</span>
                ...
                <button
                  ...
                  disabled={busyId === a.lead_id}
                  className="shrink-0 rounded-md bg-warm-600 px-3 py-1.5 text-xs font-medium text-white transition-colors hover:opacity-90 disabled:opacity-50"
                >
          ...
          {readyToClaim.length > CLAIM_PREVIEW_CAP && (
            <Link
              to="/leads?tier=HOT"
              className="mt-2 block rounded-md border border-dashed border-warm-600/30 py-2 text-center text-xs font-medium text-warm-700 transition-colors hover:border-warm-600/50 hover:bg-warm-100"
            >
