...
from database.models import Campaign, Product, TodoItem
...
def list_todos():
    ...
    `product_title` are joined in here so the frontend never needs a second round-trip per
    item just to label its card."""
    db = SessionLocal()
    try:
        items = (
            db.query(TodoItem)
